...
static esp_err_t ai_capture_handler(httpd_req_t *req)
{
    sensor_t *s = esp_camera_sensor_get();
    if (!s) {
        ESP_LOGE(TAG, "Camera sensor not found");
    ...
    // 添加分辨率信息（方便调试）
    char dim[32];
    snprintf(dim, sizeof(dim), "%dx%d", fb->width, fb->height);
    httpd_resp_set_hdr(req, "X-Resolution", dim);
    
    ESP_LOGI(TAG, "AI capture: %dx%d, %d bytes", fb->width, fb->height, fb->len);
...
//ESP32摄像头HTTP服务器的初始化代码，实现了Web界面控制摄像头功能
void app_httpd_main()
{
    ...
    // 为esp32s3获取帧图像提供专用接口
    httpd_uri_t ai_capture_uri = {
        .uri = "/ai_capture",   
        .method = HTTP_GET,
        .handler = ai_capture_handler,
        .user_ctx = NULL};

    /*
        初始化滚动平均滤波器（Rolling Average Filter）
        窗口大小：20帧
        作用：平滑图像参数，减少噪声
    */    
...
//人脸检测配置    
    ...
    mtmn_config.type = FAST;              // 使用快速检测模式
    mtmn_config.min_face = 80;            // 最小检测人脸大小：80x80像素
    mtmn_config.pyramid = 0.707;          // 图像金字塔缩放因子
    mtmn_config.pyramid_times = 4;        // 金字塔层数
    
    //三阶段检测阈值配置
    // P-Net（Proposal Network）参数
    mtmn_config.p_threshold.score = 0.6;            // 得分阈值
    mtmn_config.p_threshold.nms = 0.7;              // 非极大值抑制阈值
    mtmn_config.p_threshold.candidate_number = 20;  // 候选框数量

    // R-Net（Refine Network）参数
    mtmn_config.r_threshold.score = 0.7;
    mtmn_config.r_threshold.nms = 0.7;
    mtmn_config.r_threshold.candidate_number = 10;

    // O-Net（Output Network）参数
...
/*
    初始化人脸ID数据库
    FACE_ID_SAVE_NUMBER：最大保存人脸数
    ENROLL_CONFIRM_TIMES：注册确认次数
*/    
#if CONFIG_ESP_FACE_RECOGNITION_ENABLED
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK)
    {
        ...
        httpd_register_uri_handler(camera_httpd, &ai_capture_uri);
    }
